...
const Close = exports.Close = ({ currentWindow, isFocused, showOnHover }) => {
  const classOnFocus = isFocused ? 'macos-close-btn-focused' : '';
  const classOnHover = showOnHover ? 'show-on-hover' : '';
  return _react2.default.createElement(
    'a',
    { className: `${classOnHover} ${classOnFocus} macos-close-btn macos-control-btn`, onClick: () => currentWindow.close() },
