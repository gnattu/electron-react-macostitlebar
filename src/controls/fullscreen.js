...
const fullscreenIcon = window.devicePixelRatio > 1.5 ? (
  ...
  <svg x="0px" y="0px" width="10px" height="10px" viewBox="0 0 10 10" className = "macos-btn-icon">
...
const maximizeIcon = window.devicePixelRatio > 1.5 ? (
  <svg x="0px" y="0px" width="10px" height="10px" viewBox="0 0 20 20" className = "macos-btn-icon">
...
export class Fullscreen extends Component {
  ...
  handleKeydown = e => {e.altKey && this.setState({ altKey: true })}
  ...
  render() {
    ...
    if (process.platform === 'darwin' && allowFullscreen && !altKey) {
      icon = fullscreenIcon
      onClick = () => {currentWindow.setFullScreen(true)}
    } else {
      icon = maximizeIcon
